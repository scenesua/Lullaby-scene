...
(()=>{
  ...
  // v4 keys invalidate every previously stuck 0/0 marker.
  ...
  function render(today,total,backend){
    ...
    root.dataset.unavailable='false';root.dataset.backend=backend||'unknown';root.dataset.counterVersion='6';root.removeAttribute('title');
  }
  function lastNumber(key){const value=Number(storage.get(key));return Number.isFinite(value)&&value>0?value:0}
  async function requestCounts(countTotal,countDay){
    const response=await fetch('/api/visitors',{
      method:'POST',headers:{'Content-Type':'application/json','Accept':'application/json'},
      body:JSON.stringify({visitorId:visitorId(),countTotal,countDay}),cache:'no-store',
    });
  ...
  async function load(){
    const day=seoulDay();
    let countTotal=storage.get(TOTAL_MARKER)!=='1';
    let countDay=storage.get(DAY_MARKER)!==day;
    try{
      let result=await requestCounts(countTotal,countDay);

      // Repair stale browser markers or a reset backend. A visible 0 is never a
      // valid steady state after a real page visit: force one counted POST and
      // require the backend to confirm a positive value before rendering.
      if(result.today<1||result.total<1){
        result=await requestCounts(true,true);
        countTotal=true;countDay=true;
      }
      if(result.today<1||result.total<1)throw new Error('visitor counter repair did not produce a positive count');

      let {today,total,data}=result;
      ...
      if(data.backend==='d1'){
        // D1 deduplicates by visitor hash/day on the server.
        storage.set(TOTAL_MARKER,'1');storage.set(DAY_MARKER,day);
      }else{
        if((data.countedTotal===true||!countTotal)&&total>0)storage.set(TOTAL_MARKER,'1');
        if((data.countedDay===true||!countDay)&&today>0)storage.set(DAY_MARKER,day);
      }
      render(today,total,data.backend);
    }catch(error){
      root.dataset.unavailable='true';root.dataset.counterVersion='6';
