(()=>{
  ...
  const originalStartScene=startScene,playButton=document.getElementById('scenePlay');
  if(playButton&&originalStartScene)playButton.removeEventListener('click',originalStartScene);
  let taxiNode=null,audiblePhase='Ready';
  ...
  async function ensureJourneyNodes(){await Promise.all([ensureSceneNode(),ensureTaxiNode()])}
  ...
  updateSceneAudio=function(ms){
    ...
    const phaseDirect={'Taxi out':.50,Takeoff:.61,Climb:.57,Cruise:.53,Descent:.54,Approach:.56,Touchdown:.58,'Taxi in':.48,Arrived:0}[phase]??.53;
    const direct=Math.max(0,phaseDirect*(.88+macro.engine*.18)+(event?.type==='turbulence'?.008*macro.turbulence:0)+(event?.type==='cabin'?.006*macro.activity:0));
    ...
    sceneNode.filter.frequency.setTargetAtTime(Math.max(16500,18800-(macro.night*1100)-((1-macro.engine)*450)),ctx.currentTime,1.5);
    taxiNode.filter.frequency.setTargetAtTime(Math.max(18000,20000-(macro.night*450)),ctx.currentTime,1.5);
  };

  pauseScene=function(){
    if(!scenePlaying)return;pausedAt=currentElapsed();scenePlaying=false;sceneNode?.el.pause();taxiNode?.el.pause();clearInterval(sceneTimer);
  ...
  startScene=async function(){
    try{
      ...
      sceneStartedAt=performance.now();scenePlaying=true;await Promise.all([sceneNode.el.play(),taxiNode.el.play()]);updateSceneAudio(currentElapsed());
  ...
  function stepScenePhase(direction){
    ...
    return seekSceneToMs(Math.min(total-1,Math.round(total*phaseSteps[index][1])));
  }
  function localizePhaseButtons(){
    const en=window.LullabyI18n?.language==='en',prev=document.getElementById('journeyPrevPhase'),next=document.getElementById('journeyNextPhase');
  ...
  const baseUpdateSceneUi=updateSceneUi;updateSceneUi=function(){baseUpdateSceneUi();syncJourneySeekAria()};
