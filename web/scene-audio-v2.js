(()=>{
  ...
  function syncEventToggle(){const button=document.getElementById('journeyEventToggle'),label=document.querySelector('[data-journey-event-label]'),en=english(),name=en?'Random events':'랜덤 이벤트';if(label)label.textContent=name;if(button){button.setAttribute('aria-label',name);button.setAttribute('aria-checked',String(journeyEventsEnabled));button.textContent=journeyEventsEnabled?(en?'On':'켜짐'):(en?'Off':'꺼짐')}}
