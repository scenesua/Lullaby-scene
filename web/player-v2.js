...
const builtinPresets=[
 {id:'preset_rain_eaves',name:'Rain Beneath the Eaves',master:.65,mix:{rain:.45,rain_drum:.28}},
...
function updateSceneUi(){const elapsed=currentElapsed(),total=durationMinutes*60000,remaining=Math.max(0,total-elapsed),[phase,belt]=phaseFor(elapsed,total),ev=activeSceneEvent(elapsed),phaseLabel=$('#phaseLabel');if(phaseLabel){phaseLabel.dataset.phaseKey=phase;phaseLabel.textContent=window.LullabyCatalogI18n?.phaseName?.(phase)||phase}$('#elapsedLabel').textContent=fmt(elapsed,true);$('#remainingLabel').textContent=fmt(remaining);$('#seatbeltLabel').textContent=belt?'ON':'OFF';$('#journeyProgress').style.width=`${Math.min(100,elapsed/total*100)}%`;$('#eventLabel').textContent=ev?(ev.type==='turbulence'?'Light turbulence':'Quiet cabin movement'):'None';updateSceneAudio(elapsed);updateNowPlaying();if(elapsed>=total)stopScene(true)}
