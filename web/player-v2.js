...
function makeCrossfadeLoopNode(url,{durationSeconds,fadeSeconds=8,preload='auto'}={}){
  const duration=Math.max(.5,Number(durationSeconds)||0),fade=clamp(Number(fadeSeconds)||8,.1,Math.max(.1,duration/3));
  const input=ctx.createGain(),filter=ctx.createBiquadFilter(),gain=ctx.createGain();filter.type='lowpass';filter.frequency.value=20000;gain.gain.value=.3;input.connect(filter).connect(gain).connect(master);
  const voices=Array.from({length:2},()=>{const el=new Audio();el.loop=false;el.preload=preload;el.crossOrigin='anonymous';el.src=url;const source=ctx.createMediaElementSource(el),envelope=ctx.createGain();envelope.gain.value=0;source.connect(envelope).connect(input);return{el,source,envelope}});
  const curveLength=33,outCurve=new Float32Array(curveLength),inCurve=new Float32Array(curveLength);for(let i=0;i<curveLength;i++){const t=i/(curveLength-1);outCurve[i]=Math.cos(t*Math.PI/2);inCurve[i]=Math.sin(t*Math.PI/2)}
  let active=0,playing=false,transitioning=false,timer=null,cleanupTimer=null,generation=0,loopCount=0;
  const clearTimers=()=>{clearTimeout(timer);clearTimeout(cleanupTimer);timer=null;cleanupTimer=null};
  const setEnvelope=(index,value)=>{const param=voices[index].envelope.gain;param.cancelScheduledValues(ctx.currentTime);param.setValueAtTime(value,ctx.currentTime)};
  const arm=()=>{clearTimeout(timer);timer=null;if(!playing||transitioning)return;const remaining=Math.max(.05,duration-(Number(voices[active].el.currentTime)||0)-fade);timer=setTimeout(beginCrossfade,remaining*1000)};
  const recover=async index=>{if(!playing||transitioning||index!==active)return;const token=++generation,next=1-index;setEnvelope(index,0);setEnvelope(next,1);try{voices[next].el.currentTime=0;await voices[next].el.play();if(token!==generation||!playing){voices[next].el.pause();return}active=next;loopCount++;arm()}catch(error){timer=setTimeout(()=>recover(index),250);console.warn('Loop recovery delayed',error)}};
  async function beginCrossfade(){
    if(!playing||transitioning)return;transitioning=true;clearTimeout(timer);timer=null;const token=++generation,outgoing=active,incoming=1-outgoing,outVoice=voices[outgoing],inVoice=voices[incoming];
    try{inVoice.el.pause();inVoice.el.currentTime=0;setEnvelope(incoming,0);await inVoice.el.play()}catch(error){transitioning=false;timer=setTimeout(beginCrossfade,250);console.warn('Loop crossfade delayed',error);return}
    if(token!==generation||!playing){inVoice.el.pause();transitioning=false;return}
    const now=ctx.currentTime;outVoice.envelope.gain.cancelScheduledValues(now);inVoice.envelope.gain.cancelScheduledValues(now);outVoice.envelope.gain.setValueCurveAtTime(outCurve,now,fade);inVoice.envelope.gain.setValueCurveAtTime(inCurve,now,fade);active=incoming;loopCount++;transitioning=false;arm();
    cleanupTimer=setTimeout(()=>{outVoice.el.pause();try{outVoice.el.currentTime=0}catch{};setEnvelope(outgoing,0)},fade*1000+100);
  }
  voices.forEach((voice,index)=>{voice.el.addEventListener('timeupdate',()=>{if(playing&&!transitioning&&index===active&&duration-voice.el.currentTime<=fade+.15)void beginCrossfade()});voice.el.addEventListener('ended',()=>void recover(index))});
  const el={
    get paused(){return!playing},get ended(){return false},get currentTime(){return Number(voices[active].el.currentTime)||0},set currentTime(value){const position=((Number(value)||0)%duration+duration)%duration,wasPlaying=playing;generation++;clearTimers();transitioning=false;voices.forEach(voice=>voice.el.pause());setEnvelope(active,1);setEnvelope(1-active,0);voices[active].el.currentTime=position;if(wasPlaying)voices[active].el.play().then(arm).catch(error=>{playing=false;console.warn('Loop seek failed',error)})},
    get duration(){return duration},get preload(){return voices[active].el.preload},set preload(value){voices.forEach(voice=>voice.el.preload=value)},get loop(){return true},set loop(_) {},
    async play(){if(playing&&!voices[active].el.paused)return;playing=true;const token=++generation;setEnvelope(active,1);setEnvelope(1-active,0);try{await voices[active].el.play();if(token!==generation||!playing){voices[active].el.pause();return}arm()}catch(error){playing=false;throw error}},
    pause(){if(!playing&&voices.every(voice=>voice.el.paused))return;playing=false;generation++;transitioning=false;clearTimers();voices.forEach(voice=>voice.el.pause())}
  };
  return{el,src:input,input,sources:voices.map(voice=>voice.source),filter,gain,url,voices,__lullabyCrossfadeLoop:true,get loopCount(){return loopCount},loopDurationSeconds:duration,loopFadeSeconds:fade};
}
window.LullabyLoopCrossfade={makeNode:makeCrossfadeLoopNode,gains:t=>{const value=clamp(Number(t)||0,0,1);return[Math.cos(value*Math.PI/2),Math.sin(value*Math.PI/2)]}};
...
function renderMixer(){const journey=catalog.filter(def=>journeyEventIds.has(def.id)),regular=catalog.filter(def=>!journeyEventIds.has(def.id)),list=currentFilter==='all'?regular:currentFilter==='journeyEvents'?[]:regular.filter(def=>def.category===currentFilter),journeyList=currentFilter==='all'||currentFilter==='journeyEvents'?journey:[];const card=def=>{const st=getMixerUiState(def.id),kind=def.kind==='event'?'event layer':'continuous',classes=['mixer-source',st.on?'on':'',journeyEventIds.has(def.id)?'journey-event':'',hoodJourneyEventIds.has(def.id)?'hood-journey-event':''].filter(Boolean).join(' ');return`<article class="${classes}" data-source="${def.id}"><div><strong>${def.name}</strong><span>${journeyEventIds.has(def.id)?'journey event':`${def.category} · ${kind}`}</span></div><button type="button" data-source-toggle="${def.id}">${st.on?'On':'Off'}</button><input data-source-volume="${def.id}" type="range" min="0" max="100" value="${st.volume}" aria-label="${def.name} volume"></article>`};$('#mixerGrid').innerHTML=list.map(card).join('')+(journeyList.length?`<div class="mixer-group-heading" data-mixer-group-title>Journey events</div>${journeyList.map(card).join('')}`:'');$$('[data-source-toggle]').forEach(b=>b.addEventListener('click',()=>toggleMixer(b.dataset.sourceToggle)));$$('[data-source-volume]').forEach(r=>r.addEventListener('input',e=>setSourceVolume(e.target.dataset.sourceVolume,+e.target.value/100)));window.LullabyCatalogI18n?.apply?.()}
...
async function ensureSceneNode(){await ensureContext();if(sceneNode)return sceneNode;setStatus('Passenger Aircraft Cabin 오디오를 준비하는 중…');sceneNode=makeCrossfadeLoopNode(await getAircraftUrl(),{durationSeconds:window.LullabyAircraftSource?.durationSeconds||105,fadeSeconds:3.5});sceneNode.gain.gain.value=.55;return sceneNode}
...
const USER_PRESET_ID=/^user_[A-Za-z0-9_-]+$/;
function loadUserPresets(){try{const list=JSON.parse(localStorage.getItem('lullaby-user-presets')||'[]');return Array.isArray(list)?list.filter(p=>p&&typeof p==='object'&&USER_PRESET_ID.test(String(p.id||''))&&p.mix&&typeof p.mix==='object'&&!Array.isArray(p.mix)):[]}catch{return[]}}
function saveUserPresets(list){localStorage.setItem('lullaby-user-presets',JSON.stringify(list));renderUserPresets()}
function renderUserPresets(){const root=$('#userPresets');if(!root)return;const list=loadUserPresets();root.replaceChildren();if(!list.length){const empty=document.createElement('p');empty.className='muted-copy';empty.textContent='저장한 프리셋이 없습니다.';root.appendChild(empty);return}for(const p of list){const card=document.createElement('div');card.className='preset-card user';const open=document.createElement('button');open.type='button';open.dataset.userPreset=p.id;const name=document.createElement('strong');name.textContent=String(p.name??'');const count=document.createElement('span');count.textContent=`${Object.keys(p.mix).length} sources`;open.append(name,count);open.addEventListener('click',()=>applyPreset(p.id));const remove=document.createElement('button');remove.type='button';remove.className='preset-delete';remove.dataset.deletePreset=p.id;remove.setAttribute('aria-label','삭제');remove.textContent='×';remove.addEventListener('click',()=>saveUserPresets(loadUserPresets().filter(item=>item.id!==p.id)));card.append(open,remove);root.appendChild(card)}}
