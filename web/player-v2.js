...
async function loadCatalog(){const res=await fetch('/mixer-sources.json',{cache:'no-cache'});if(!res.ok)throw new Error('mixer catalog');const data=await res.json();const preview=['localhost','127.0.0.1','debug.lullabyscene.com','lullaby-scene-debug.pages.dev'].includes(location.hostname)||location.hostname.endsWith('.lullaby-scene-debug.pages.dev');catalog=data.sources.filter(def=>!def.previewOnly||preview);sourceById=Object.fromEntries(catalog.map(s=>[s.id,s]));renderMixerFilters();renderMixer();renderPresets()}
