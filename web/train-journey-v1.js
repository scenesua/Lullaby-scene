(()=>{
  ...
  function roleFor(ms,total){const phase=trainPhaseFor(ms,total)[0];return phase==='Departing'?'departure':phase==='Arriving'?'arrival':phase==='Arrived'?null:'bed'}
  ...
  function updateTrainUi(){
    ...
    const values={phaseLabel:phase,elapsedLabel:fmt(elapsed,true),remainingLabel:fmt(remaining),seatbeltLabel:phase==='Arrived'?(isEnglish()?'Arrived':'도착'):(isEnglish()?'In motion':'운행 중'),eventLabel:copy().none};
    ...
    updateTrainAudio(elapsed);updateNowPlaying();
  ...
  function trainStep(direction){
    ...
    return window.LullabyJourneyRuntime?.seekToMs(steps[index]);
  }
  ...
  function injectSelector(){
    ...
    row.addEventListener('click',event=>{const button=event.target.closest('[data-journey]');if(!button||scenePlaying)return;stopScene(false);activeJourneyId=button.dataset.journey;renderJourney()});
  ...
  function renderJourney(){
    ...
    if(!scenePlaying&&pausedAt===0){if(playButton)playButton.textContent=text.start;const event=document.getElementById('eventLabel');if(event)event.textContent=train?text.none:'None'}
  }
  injectSelector();renderJourney();
  document.addEventListener('lullaby-language-changed',()=>setTimeout(renderJourney));
  window.LullabyTrainJourney={phaseFor:trainPhaseFor,boundaries:trainBoundaries,sources:SOURCES,get active(){return isTrain()},get audibleRole(){return audibleRole}};
})();
