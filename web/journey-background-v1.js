(()=>{
  ...
  function copy(){const t=(key,fallback)=>window.LullabyLocales?.t?.(key)||fallback;return{open:t('sceneScreen','Scene Screen'),exit:t('exitSceneScreen','Exit Scene Screen'),brightness:t('brightness','Brightness'),on:t('backgroundOff','Background off'),off:t('backgroundOn','Background on')}}
  ...
  function animate(now){frame=requestAnimationFrame(animate);if(document.hidden||!ambientEnabled)return;if(now-lastFrame<33)return;const dt=Math.min(.12,Math.max(.016,(now-lastFrame)/1000||.033));lastFrame=now;const analyser=window.LullabyAudioReactive?.analyser,context=window.LullabyAudioReactive?.context;let target=0;if(analyser&&context?.state==='running'){const bins=new Uint8Array(analyser.frequencyBinCount);analyser.getByteFrequencyData(bins);let total=0,count=Math.min(28,bins.length);for(let i=1;i<count;i++)total+=bins[i];energy=total/Math.max(1,count-1)/255;target=Math.min(1,Math.max(0,(energy-.012)*5.2))}else energy=0;const seconds=target>envelope?1.8:4.8;envelope+=(target-envelope)*(1-Math.exp(-dt/seconds));const slow=(Math.sin(now/5100)+1)/2,drift=(Math.sin(now/8700+1.7)+1)/2;const breath=slow*.68+drift*.32,range=reduceMotion.matches?.28:1;const exposure=Math.min(1,.12+(breath*.22+envelope*.48)*range);visual.style.setProperty('--scene-exposure',exposure.toFixed(3))}
  ...
  visual.style.setProperty('--scene-exposure','.12');frame=requestAnimationFrame(animate);show();injectAmbientToggle();localize();setTimeout(injectDisplayButtons,300);setTimeout(injectDisplayButtons,1000);setTimeout(injectDisplayButtons,1800);
})();
