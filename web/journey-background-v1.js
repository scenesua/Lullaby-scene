(()=>{
  ...
  exit.className='journey-display-exit';exit.innerHTML='<button type="button"></button>';document.body.appendChild(exit);
  ...
  function copy(){const ko=(window.LullabyI18n?.language||document.documentElement.lang||'ko')==='ko';return ko?{open:'장면 화면',exit:'장면 화면 닫기',on:'배경 끄기',off:'배경 켜기'}:{open:'Scene Screen',exit:'Exit Scene Screen',on:'Background off',off:'Background on'}}
  function localize(){const text=copy();exit.querySelector('button').textContent=text.exit;document.querySelectorAll('[data-journey-display-label]').forEach(label=>label.textContent=text.open);document.querySelectorAll('[data-journey-display-button]').forEach(button=>{button.title=text.open;button.setAttribute('aria-label',text.open)});const toggle=document.querySelector('[data-journey-background-toggle]');if(toggle){toggle.innerHTML=`${ambientEnabled?'◐':'○'} <span>${ambientEnabled?text.on:text.off}</span>`;toggle.setAttribute('aria-pressed',String(ambientEnabled))}}
