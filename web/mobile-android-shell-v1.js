(()=>{
  ...
  languageLabel.ar=['اللغة','لغة عرض الموقع والمشغل'];fxCopy.ar=['FX','اضبط النغمة والمكان لصوت الخلاط والمشاهد الجاهزة.','إعادة ضبط المؤثرات'];
  const copy=map=>map[L()?.language||'en']||map.en;
  const timerLabel=()=>copy({ko:'타이머',en:'Timer',ar:'المؤقت',ja:'タイマー','zh-CN':'定时器','zh-TW':'計時器',ru:'Таймер',fr:'Minuteur',es:'Temporizador',pt:'Timer',th:'ตั้งเวลา',tl:'Timer',hi:'टाइमर',vi:'Hẹn giờ'});
  const displayLabel=()=>copy({ko:'장면 화면',en:'Scene Screen',ar:'شاشة المشهد',ja:'シーン画面','zh-CN':'场景画面','zh-TW':'場景畫面',ru:'Сцена',fr:'Scène',es:'Escena',pt:'Cena',th:'ฉาก',tl:'Eksena',hi:'दृश्य',vi:'Cảnh'});
  const journeyLabel=()=>({ko:'여정',en:'Journey',ar:'الرحلة',ja:'旅','zh-CN':'旅程','zh-TW':'旅程',ru:'Путешествие',fr:'Voyage',es:'Viaje',pt:'Jornada',th:'การเดินทาง',tl:'Paglalakbay',hi:'यात्रा',vi:'Hành trình'}[L()?.language||'en']||'Journey');
  ...
  function updateChrome(){
    ...
    back?.setAttribute('aria-label',term('back','Back'));$('[data-android-timer]')?.setAttribute('aria-label',t('timer','Sleep timer'));setText($('[data-timer-label]'),timerLabel());setText($('[data-mobile-display-label]'),displayLabel());
