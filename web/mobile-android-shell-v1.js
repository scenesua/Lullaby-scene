(()=>{
  ...
  function ensureBottomNav(){
    ...
    nav.innerHTML=[['scenes','✦'],['mixer','≋'],['prepared','▣'],['fx','◌'],['settings','⚙']].map(([id,icon])=>`<button class="mobile-tab" type="button" data-android-dest="${id}"><span aria-hidden="true">${icon}</span><b></b></button>`).join('');
  ...
  function destinationTitle(dest){return dest==='scenes'?t('scenes','Scenes'):dest==='mixer'?t('mixer','Mixer'):dest==='prepared'?t('simpleScenes','Ready-made Scenes'):dest==='fx'?term('fx','FX'):dest==='settings'?t('settings','Settings'):dest==='timer'?t('timer','Sleep Timer'):t('scenes','Scenes')}
  function updateChrome(){
    ...
    $$('[data-android-dest]').forEach(btn=>{const id=btn.dataset.androidDest,labels={scenes:t('scenes','Scenes'),mixer:t('mixer','Mixer'),prepared:t('simpleScenes','Ready-made Scenes'),fx:term('fx','FX'),settings:t('settings','Settings')};btn.classList.toggle('active',id===activeDest);btn.setAttribute('aria-current',id===activeDest?'page':'false');setText(btn.querySelector('b'),labels[id])});
    const [langTitle,langCopy]=copy(languageLabel);setText($('[data-language-title]'),langTitle);setText($('[data-language-copy]'),langCopy);
  ...
  function syncPlayButton(){const button=$('[data-android-play]');if(!button)return;const journeyPlaying=activeDest==='scenes'&&(document.getElementById('scenePlay')?.textContent||'').includes('Ⅱ');const mixPlaying=activeDest!=='scenes'&&mixerStates().length>0&&!pausedMixer.length;const playing=journeyPlaying||mixPlaying;setText(button,playing?'Ⅱ':'▶');button.setAttribute('aria-label',playing?term('pause','Pause'):pausedMixer.length?term('resume','Resume'):'Play')}
