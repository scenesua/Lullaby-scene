(()=>{
  ...
  function updateChrome(){
    ...
    back?.setAttribute('aria-label',term('back','Back'));$('[data-android-timer]')?.setAttribute('aria-label',t('timer','Sleep timer'));
  ...
  function syncPlayButton(){const button=$('[data-android-play]');if(!button)return;const journeyRunning=(document.getElementById('scenePlay')?.textContent||'').includes('Ⅱ');const journeyPlaying=activeDest==='scenes'&&journeyRunning;const mixPlaying=activeDest!=='scenes'&&mixerStates().length>0&&!pausedMixer.length;const playing=journeyPlaying||mixPlaying;setText(button,playing?'Ⅱ':'▶');button.setAttribute('aria-label',playing?term('pause','Pause'):pausedMixer.length?term('resume','Resume'):t('start','Play'));$('#webPlayer').dataset.journeyPlaying=String(journeyRunning);$$('.android-icon-button,.mobile-scene-display-button').forEach(control=>{control.title=control.getAttribute('aria-label')||''})}
