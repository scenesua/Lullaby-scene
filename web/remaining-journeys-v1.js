(()=>{
  ...
  async function debugTriggerEvent(type='random'){
    ...
    if(activeJourneyId==='hood_journey'){
      ...
      if(type!=='random'){if(type==='siren'){const startPan=random(-.96,-.72),targetPan=random(-.55,.55);await playHoodEvent(type,random(.96,1),startPan,{pan:targetPan,distance:random(.68,.84),endPan:random(.82,.98),endDistance:random(.97,1)})}else await playHoodEvent(type,random(type==='glass'?.68:type==='dog'?.45:.58,type==='dog'?.72:1));return local(hoodLabels[type]||[type,type])}
      const intensity=.35+.65*macro.turbulence;if(Math.random()<.55){startHoodFight(intensity);return'HOOD random fight'}
      const calm=['footsteps','carPass','carDoor','dog','dog','helicopter','glass'],picked=calm[randomInt(0,calm.length)];await playHoodEvent(picked,random(picked==='carPass'?.18:picked==='glass'?.68:picked==='dog'?.45:.58,picked==='dog'?.80:1));return local(hoodLabels[picked])
    }
