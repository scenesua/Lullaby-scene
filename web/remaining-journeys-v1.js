(()=>{
  ...
  const media={},eventNodes={};let audibleRole=null,eventTimer=null,eventLabelUntil=0,roleGeneration=0;
  ...
  async function ensureNodes(){
    ...
    if(cfg.event&&!eventNodes[activeJourneyId]){eventNodes[activeJourneyId]=makeMediaNode(cfg.event.url,{loop:false,preload:'none'});eventNodes[activeJourneyId].gain.gain.value=.08}
  }
  function activeNodes(){const cfg=config();return cfg?media[cfg.title[0]]||{}:{}}
  function pauseEvent(){clearTimeout(eventTimer);eventTimer=null;eventLabelUntil=0;for(const node of Object.values(eventNodes)){node.el.pause();try{node.el.currentTime=0}catch{}}}
  ...
  function updateAudio(ms){
    ...
    const eventNode=eventNodes[activeJourneyId];if(eventNode)eventNode.gain.gain.setTargetAtTime(.08*(.65+.35*macro.activity)*(1-.08*macro.night),ctx.currentTime,.5);
  }
  function scheduleEvent(delayMs){clearTimeout(eventTimer);const cfg=config();if(!cfg?.event||!scenePlaying||!window.LullabyJourneyEvents?.enabled)return;const id=activeJourneyId,event=cfg.event,delay=delayMs??event.minMs+Math.random()*(event.maxMs-event.minMs);eventTimer=setTimeout(async()=>{if(activeJourneyId!==id||!scenePlaying||!window.LullabyJourneyEvents?.enabled)return;const index=phaseIndex(currentElapsed(),Math.max(60000,durationMinutes*60000),cfg);if(!event.phases.includes(index)){scheduleEvent(90000);return}try{const node=eventNodes[id];node.el.currentTime=0;eventLabelUntil=performance.now()+event.durationMs;await node.el.play()}catch(error){console.warn(`${id} event unavailable`,error)}scheduleEvent()},delay)}
  function updateUi(){
    ...
    const eventLabel=!window.LullabyJourneyEvents?.enabled?(korean()?'꺼짐':'Off'):performance.now()<eventLabelUntil?local(cfg.event.label):local(cfg.detail),values={phaseLabel:phase,elapsedLabel:fmt(elapsed,true),remainingLabel:fmt(remaining),seatbeltLabel:korean()?'운행 중':'In motion',eventLabel};
