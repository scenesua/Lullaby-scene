(()=>{
  ...
  startScene=async function(){if(playButton?.getAttribute('aria-busy')==='true')return;playButton?.setAttribute('aria-busy','true');if(playButton)playButton.disabled=true;try{return await(config()?start():base.start())}finally{playButton?.removeAttribute('aria-busy');if(playButton)playButton.disabled=false}};pauseScene=function(){return config()?pause():base.pause()};stopScene=function(arrived=false){return config()?stop(arrived):base.stop(arrived)};phaseFor=function(ms,total){const info=phaseInfo(ms,total);return info?[phaseName(info[0]),false]:base.phaseFor(ms,total)};updateSceneAudio=function(ms){return config()?updateAudio(ms):base.updateAudio(ms)};updateSceneUi=function(){return config()?updateUi():base.updateUi()};if(playButton)playButton.addEventListener('click',startScene);
