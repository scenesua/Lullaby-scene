(()=>{
  ...
  async function playHoodEvent(type,distance=Math.random(),panOverride=null,target=null){
    if(activeJourneyId!=='hood_journey'||!scenePlaying||!window.LullabyJourneyEvents?.enabled)return;const node=hoodEventNode(type);if(!node)return;const gun=hoodGuns.includes(type),voice=hoodVoices.includes(type),sirenType=hoodSirens.includes(type),minimumDistance=sirenType?.62:gun?.70:type==='glass'?.68:voice?.58:0,d=Math.max(minimumDistance,Math.min(1,distance)),close=1-d,base=gun?0.13:voice?0.12:sirenType?0.20:{glass:.10,footsteps:.08,carPass:.12,carDoor:.10,helicopter:.09,dog:.08}[type]||.1,range=gun?0.20:voice?0.18:sirenType?0.18:{glass:.18,footsteps:.11,carPass:.18,carDoor:.17,helicopter:.13,dog:.14}[type]||.15;
    const startPan=panOverride==null?random(-.85,.85):Math.max(-1,Math.min(1,panOverride)),targetDistance=Math.max(minimumDistance,Math.min(1,target?.distance??d)),targetClose=1-targetDistance,targetPan=Math.max(-1,Math.min(1,target?.pan??startPan)),duration=(config().event.sources[type]?.[1]||2500)/1000,frequency=sirenType?2400+close*4200:(gun||voice?2600:3200)+close*(gun||voice?5200:10800),targetFrequency=sirenType?2400+targetClose*4200:frequency,gain=(base+close*range)*(.78+.35*macro.activity),targetGain=(base+targetClose*range)*(.78+.35*macro.activity);
    node.filter.frequency.cancelScheduledValues(ctx.currentTime);node.filter.frequency.setValueAtTime(frequency,ctx.currentTime);node.gain.gain.cancelScheduledValues(ctx.currentTime);node.gain.gain.setValueAtTime(gain,ctx.currentTime);if(sirenType&&target){node.filter.frequency.linearRampToValueAtTime(targetFrequency,ctx.currentTime+duration);node.gain.gain.linearRampToValueAtTime(targetGain,ctx.currentTime+duration)}if(node.panner){const direction=Math.random()<.5?-1:1;node.panner.pan.cancelScheduledValues(ctx.currentTime);node.panner.pan.setValueAtTime(type==='carPass'?0.92*direction:startPan,ctx.currentTime);if(type==='carPass')node.panner.pan.linearRampToValueAtTime(-.92*direction,ctx.currentTime+6.25);else if(sirenType&&target)node.panner.pan.linearRampToValueAtTime(targetPan,ctx.currentTime+duration)}try{node.el.currentTime=0;currentEventLabel=hoodLabels[type];eventLabelUntil=performance.now()+(config().event.sources[type]?.[1]||2500);await node.el.play();if(sirenType)document.dispatchEvent(new CustomEvent('lullaby-hood-siren',{detail:{distance:d,durationMs:duration*1000,direction:targetPan>=startPan?1:-1}}))}catch(error){console.warn(`hood ${type} unavailable`,error)}
  }
  function startHoodFight(intensity){
    ...
    const sirenCount=randomInt(1,4),delays=Array.from({length:sirenCount},()=>lastGunshotMs+random(18000,115001)).sort((a,b)=>a-b);delays.forEach((delay,index)=>{const targetPan=Math.max(-.88,Math.min(.88,lastGunSide.pan+random(-.10,.10))),targetDistance=Math.max(.62,Math.min(1,lastGunSide.distance+random(.04,.14)));let startPan=random(-.92,.92);if(Math.abs(startPan-targetPan)<.32)startPan=targetPan>=0?random(-.92,-.18):random(.18,.92);later(()=>void playHoodEvent(hoodSirens[index],random(.78,1),startPan,{pan:targetPan,distance:targetDistance}),delay)});
  }
  function scheduleHoodEvent(delayMs){const cfg=config(),id=activeJourneyId,event=cfg?.event;if(id!=='hood_journey'||!event)return;later(()=>{if(activeJourneyId!==id||!scenePlaying||!window.LullabyJourneyEvents?.enabled)return;const index=phaseIndex(currentElapsed(),Math.max(60000,durationMinutes*60000),cfg);if(!event.phases.includes(index)){scheduleHoodEvent(90000);return}const intensity=.35+.65*macro.turbulence;if(Math.random()<.26+.18*intensity)startHoodFight(intensity);else{const calm=['footsteps','carDoor','dog','helicopter','glass'],type=calm[Math.floor(Math.random()*calm.length)];void playHoodEvent(type,random(type==='glass'?.68:.25,1))}scheduleHoodEvent(random(event.minMs,event.maxMs))},delayMs??random(event.minMs,event.maxMs))}
  ...
  async function debugTriggerEvent(type='random'){
    ...
    if(activeJourneyId==='hood_journey'){
      ...
      if(type!=='random'){if(type==='siren'){const startPan=random(-.92,.92);await playHoodEvent(type,random(.78,1),startPan,{pan:random(-.70,.70),distance:random(.62,.84)})}else await playHoodEvent(type,random(type==='glass'?.68:.58,1));return local(hoodLabels[type]||[type,type])}
      const intensity=.35+.65*macro.turbulence;if(Math.random()<.55){startHoodFight(intensity);return'HOOD random fight'}
      const calm=['footsteps','carPass','carDoor','dog','helicopter','glass'],picked=calm[randomInt(0,calm.length)];await playHoodEvent(picked,random(picked==='carPass'?.18:picked==='glass'?.68:.58,1));return local(hoodLabels[picked])
    }
