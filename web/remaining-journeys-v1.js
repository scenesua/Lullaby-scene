(()=>{
  ...
  const configs={
    ...
    forest_temple_journey:{
      ...
      nodes:{departure:['/audio/scenes/forest_temple_journey/forest_temple_path_walk_001.ogg?v=1',104.007],forest:['/audio/scenes/forest_temple_journey/forest_temple_forest_bed_debug_004.ogg?v=1',73.220],birds:['/audio/scenes/forest_temple_journey/forest_temple_korean_distant_birds_debug_007.ogg?v=1',120.030],bowl:['/audio/scenes/forest_temple_journey/forest_temple_bowl_distant_bed_001.ogg?v=1',77.007]},roles:{departure:['departure','forest','birds'],bed:['forest','birds','bowl'],arrival:['forest','birds','bowl']},gains:{departure:1,forest:.121,birds:.11,bowl:.227},bedStartsAtPhaseBoundary:true,event:{sequence:'temple',label:['Temple sound','절의 소리'],minMs:210000,maxMs:540000,phases:[2,3],sources:{moktak:['/audio/scenes/forest_temple_journey/forest_temple_moktak_event_001.ogg?v=2',77165],gravel:['/audio/scenes/forest_temple_journey/forest_temple_gravel_event_001.ogg?v=2',26907],heartSutra:['/audio/scenes/forest_temple_journey/forest_temple_heart_sutra_event_001.ogg?v=4',120196]}},
  ...
  function configureTempleEventMix(node,type,durationMs){
    const sutra=type==='heartSutra',fromTemple=sutra||type==='moktak',now=ctx.currentTime,gain=sutra?.157:type==='gravel'?.075:.09;
    node.filter.frequency.setValueAtTime(sutra?6100:type==='gravel'?5200:7200,now);node.gain.gain.cancelScheduledValues(now);node.gain.gain.setValueAtTime(gain,now);
    if(sutra){const seconds=durationMs/1000;node.gain.gain.setValueAtTime(gain,now+Math.min(40,seconds*.34));node.gain.gain.linearRampToValueAtTime(.178,now+Math.min(82,seconds*.70));node.gain.gain.linearRampToValueAtTime(.198,now+Math.max(1,seconds-2))}
    if(node.panner)node.panner.pan.setValueAtTime(fromTemple?.48:random(-.28,.28),now);
  }
  ...
  async function playTempleEventNow(type,{resumeRandom=false}={}){
    ...
    const durationMs=source[1];configureTempleEventMix(node,type,durationMs);
  ...
  function scheduleTempleEvent(delayMs){const cfg=config(),id=activeJourneyId,event=cfg?.event;if(id!=='forest_temple_journey'||event?.sequence!=='temple')return;later(async()=>{if(activeJourneyId!==id||!scenePlaying||!window.LullabyJourneyEvents?.enabled)return;const index=phaseIndex(currentElapsed(),Math.max(60000,durationMinutes*60000),cfg);if(!event.phases.includes(index)){scheduleTempleEvent(90000);return}const available=templeEventTypes.filter(type=>event.sources[type]&&eventNodes[id]?.[type]);if(!available.length)return;const type=available[randomInt(0,available.length)],node=eventNodes[id][type],durationMs=event.sources[type][1];try{node.el.currentTime=0;configureTempleEventMix(node,type,durationMs);currentEventLabel=templeLabels[type];eventLabelUntil=performance.now()+durationMs;await node.el.play()}catch(error){console.warn(`temple ${type} unavailable`,error)}later(()=>scheduleTempleEvent(),durationMs)},delayMs??random(event.minMs,event.maxMs))}
  ...
  async function debugTriggerEvent(type='random'){
    ...
    if(activeJourneyId==='forest_temple_journey'){
      ...
      const node=eventNodes[activeJourneyId][picked],durationMs=cfg.event.sources[picked][1];configureTempleEventMix(node,picked,durationMs);currentEventLabel=templeLabels[picked];eventLabelUntil=performance.now()+durationMs;await node.el.play();updateUi();return local(templeLabels[picked])
    }
