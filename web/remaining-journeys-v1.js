(()=>{
  ...
  const configs={
    ...
    forest_temple_journey:{
      ...
      nodes:{departure:['/audio/scenes/forest_temple_journey/forest_temple_path_walk_001.ogg?v=1',104.007],forest:['/audio/scenes/forest_temple_journey/forest_temple_forest_bed_001.ogg?v=3',226],birds:['/audio/scenes/forest_temple_journey/forest_temple_korean_distant_birds_bed_001.ogg?v=1',120.030],bowl:['/audio/scenes/forest_temple_journey/forest_temple_bowl_distant_bed_001.ogg?v=1',77.007]},roles:{departure:['departure','forest','birds'],bed:['forest','birds','bowl'],arrival:['forest','birds','bowl']},gains:{departure:1,forest:.121,birds:.11,bowl:.227},bedStartsAtPhaseBoundary:true,event:{sequence:'temple',label:['Temple sound','절의 소리'],minMs:210000,maxMs:540000,phases:[2,3],sources:{moktak:['/audio/scenes/forest_temple_journey/forest_temple_moktak_event_001.ogg?v=2',77165],gravel:['/audio/scenes/forest_temple_journey/forest_temple_gravel_event_001.ogg?v=2',26907],heartSutra:['/audio/scenes/forest_temple_journey/forest_temple_heart_sutra_event_001.ogg?v=5',120196]}},
  ...
  const local=value=>{if(!Array.isArray(value))return value;const base=languageBase();if(base==='ko')return value[1]||value[0];if(base==='en')return value[0]||value[1];return window.LullabyLocales?.t?.('randomEvents')||value[0]||value[1]};
  ...
  function stagePoints(total,cfg=config()){if(!cfg)return[];const b=boundaries(total,cfg),label=value=>[value,value];return[{label:label(cfg.phases[0]),ms:0},{label:label(cfg.phases[1]),ms:b.departure},{label:label(cfg.phases[2]),ms:b.settle},{label:label(cfg.phases[3]),ms:b.approach},{label:label(cfg.phases[4]),ms:b.arrival}]}
  ...
  function updateUi(){
    ...
    for(const [id,value] of Object.entries(values)){const el=document.getElementById(id);if(el)el.textContent=value}const phaseLabel=document.getElementById('phaseLabel');if(phaseLabel)phaseLabel.dataset.phaseKey=info[0];const progress=document.getElementById('journeyProgress');if(progress)progress.style.width=`${Math.min(100,elapsed/total*100)}%`;
  ...
  async function start(){try{await ensureNodes();if(scenePlaying){pause();return}sceneStartedAt=performance.now();scenePlaying=true;updateAudio(currentElapsed());scheduleEvent();if(playButton)playButton.textContent=`Ⅱ ${window.LullabyLocales?.term?.('pause')||'Pause'}`;setStatus(localizedConfig()?.title||local(config().title));clearInterval(sceneTimer);sceneTimer=setInterval(updateSceneUi,1000);updateSceneUi()}catch(error){console.error(error);scenePlaying=false;roleGeneration++;pauseNodes();audibleRole=null;setStatus(korean()?'여정 오디오를 시작하지 못했습니다.':'Could not start journey audio.') }}
  function pause(){if(!scenePlaying)return;pausedAt=currentElapsed();scenePlaying=false;roleGeneration++;pauseNodes();audibleRole=null;clearInterval(sceneTimer);if(playButton)playButton.textContent=`▶ ${window.LullabyLocales?.term?.('resume')||'Resume'}`;setStatus(window.LullabyLocales?.term?.('pause')||'Paused');updateNowPlaying()}
  function stop(arrived=false){scenePlaying=false;pausedAt=0;templePhaseIndex=-1;templeStageThreeStarting=false;clearInterval(sceneTimer);roleGeneration++;pauseNodes(true);audibleRole=null;if(playButton)playButton.textContent=window.LullabyLocales?.t?.('startScene')||(korean()?'▶ 장면 시작':'▶ Start journey');const cfg=config(),translated=localizedConfig(),values={phaseLabel:arrived?(window.LullabyLocales?.term?.('arrived')||(korean()?'도착':'Arrived')):(window.LullabyLocales?.term?.('ready')||'Ready'),elapsedLabel:'00:00',remainingLabel:fmt(durationMinutes*60000),seatbeltLabel:'—',eventLabel:window.LullabyJourneyEvents?.enabled&&cfg?(translated?.detail||local(cfg.detail)):(window.LullabyLocales?.term?.('off')||'Off')};for(const[id,value]of Object.entries(values)){const el=document.getElementById(id);if(el)el.textContent=value}const progress=document.getElementById('journeyProgress');if(progress)progress.style.width='0';setStatus(arrived?(korean()?'여정이 종료되었습니다.':'Journey complete.'):(window.LullabyLocales?.term?.('stopped')||'Stopped'));updateNowPlaying()}
  startScene=async function(){if(playButton?.getAttribute('aria-busy')==='true')return;playButton?.setAttribute('aria-busy','true');if(playButton)playButton.disabled=true;try{return await(config()?start():base.start())}finally{playButton?.removeAttribute('aria-busy');if(playButton)playButton.disabled=false}};pauseScene=function(){return config()?pause():base.pause()};stopScene=function(arrived=false){if(!config())return base.stop(arrived);const id=activeJourneyId,result=stop(arrived);document.dispatchEvent(new CustomEvent('lullaby-journey-stopped',{detail:{id,arrived}}));return result};phaseFor=function(ms,total){const info=phaseInfo(ms,total);return info?[phaseName(info[0]),false]:base.phaseFor(ms,total)};updateSceneAudio=function(ms){return config()?updateAudio(ms):base.updateAudio(ms)};updateSceneUi=function(){return config()?updateUi():base.updateUi()};if(playButton)playButton.addEventListener('click',startScene);
  ...
  function render(){const t=(key,fallback)=>window.LullabyLocales?.t?.(key)||fallback,labels={ferry_journey:t('ferryJourney','Ferry'),spacecraft_journey:t('spacecraftJourney','Spacecraft'),submarine_journey:t('submarineJourney','Submarine'),forest_temple_journey:t('templeJourney','Forest Temple'),hood_journey:'HOOD'};for(const[id,value]of Object.entries(labels)){const span=document.querySelector(`[data-journey="${id}"] span`);if(span)span.textContent=value}const cfg=config();if(!cfg)return;const translated=localizedConfig(),titleText=translated?.title||local(cfg.title),descriptionText=translated?.description||local(cfg.description),macros=translated?.macros||cfg.macros.map(local);document.body.classList.toggle('journey-hood-active',activeJourneyId==='hood_journey');document.querySelectorAll('[data-journey]').forEach(button=>{button.classList.toggle('active',button.dataset.journey===activeJourneyId);button.setAttribute('aria-pressed',String(button.dataset.journey===activeJourneyId))});const title=document.querySelector('.aircraft-title-row h3'),desc=document.querySelector('.aircraft-title-row p'),icon=document.querySelector('.aircraft-icon');if(title)title.textContent=titleText;if(desc)desc.textContent=descriptionText;if(icon)icon.textContent=cfg.icon;document.querySelectorAll('[data-inspector-mode="journey"] h3').forEach(el=>el.textContent=titleText);const status=document.querySelectorAll('.journey-status small');if(status[0])status[0].textContent=translated?.phaseLabel||(korean()?'여정 단계':'Journey phase');if(status[3])status[3].textContent=translated?.stateLabel||(activeJourneyId==='hood_journey'?(korean()?'거리 상태':'Street state'):activeJourneyId==='forest_temple_journey'?(korean()?'절의 기척':'Temple ambience'):(korean()?'객실 상태':'Cabin state'));document.querySelectorAll('.mobile-macros label span,.desktop-macros label span').forEach((el,index)=>el.textContent=macros[index%4]);const event=document.getElementById('eventLabel');if(event)event.textContent=window.LullabyJourneyEvents?.enabled?(translated?.detail||local(cfg.detail)):(window.LullabyLocales?.term?.('off')||'Off');if(!scenePlaying&&pausedAt===0){if(playButton)playButton.textContent=t('startScene','▶ Start journey');document.getElementById('phaseLabel').textContent=window.LullabyLocales?.term?.('ready')||'Ready'}window.LullabyJourneyStageControl?.render()}
