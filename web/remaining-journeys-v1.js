(()=>{
  ...
  const media={};let audibleRole=null,lastElapsed=-1,sonarNode=null,sonarTimer=null;
  ...
  async function ensureNodes(){
    ...
    for(const [key,[url]] of Object.entries(cfg.nodes))if(!bucket[key]){const node=makeMediaNode(url,{loop:key!=='departure'&&key!=='arrival'&&key!=='transition'});node.el.preload='auto';node.gain.gain.value=0;bucket[key]=node}
    if(cfg.sonar&&!sonarNode){sonarNode=makeMediaNode(cfg.sonar,{loop:false});sonarNode.gain.gain.value=.04}
  }
  ...
  async function activateRole(role,ms,total){
    ...
    for(const node of Object.values(activeNodes())){node.el.pause();node.gain.gain.value=0}audibleRole=role;if(!role||!scenePlaying)return;
    for(const key of cfg.roles[role]){const node=activeNodes()[key];try{node.el.currentTime=offset(key,role,ms,total,cfg)}catch{}await node.el.play()}
  }
  ...
  async function start(){try{await ensureNodes();if(scenePlaying){pause();return}sceneStartedAt=performance.now();scenePlaying=true;lastElapsed=-1;updateAudio(currentElapsed());scheduleSonar();if(playButton)playButton.textContent=korean()?'Ⅱ 일시정지':'Ⅱ Pause';setStatus(`${local(config().title)} ${korean()?'재생 중':'playing'}`);clearInterval(sceneTimer);sceneTimer=setInterval(updateSceneUi,1000);updateSceneUi()}catch(error){console.error(error);scenePlaying=false;pauseNodes();setStatus(korean()?'여정 오디오를 시작하지 못했습니다.':'Could not start journey audio.') }}
  function pause(){if(!scenePlaying)return;pausedAt=currentElapsed();scenePlaying=false;pauseNodes();clearTimeout(sonarTimer);clearInterval(sceneTimer);if(playButton)playButton.textContent=korean()?'▶ 계속 재생':'▶ Resume';setStatus(korean()?'일시정지됨':'Paused');updateNowPlaying()}
  function stop(arrived=false){scenePlaying=false;pausedAt=0;clearTimeout(sonarTimer);clearInterval(sceneTimer);pauseNodes(true);audibleRole=null;lastElapsed=-1;if(playButton)playButton.textContent=korean()?'▶ 장면 시작':'▶ Start journey';const cfg=config(),values={phaseLabel:arrived?(korean()?'도착':'Arrived'):'Ready',elapsedLabel:'00:00',remainingLabel:fmt(durationMinutes*60000),seatbeltLabel:'—',eventLabel:cfg?local(cfg.detail):'None'};for(const[id,value]of Object.entries(values)){const el=document.getElementById(id);if(el)el.textContent=value}const progress=document.getElementById('journeyProgress');if(progress)progress.style.width='0';setStatus(arrived?(korean()?'여정이 종료되었습니다.':'Journey complete.'):(korean()?'정지됨':'Stopped'));updateNowPlaying()}
