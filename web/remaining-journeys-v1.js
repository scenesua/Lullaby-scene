(()=>{
  ...
  const media={},eventNodes={},hoodVoiceCursor={gunshot:0,gunShotgun:0};let audibleRole=null,eventTimers=new Set(),eventLabelUntil=0,currentEventLabel=null,roleGeneration=0;
  ...
  function stagePoints(total,cfg=config()){if(!cfg)return[];const b=boundaries(total,cfg);return[{label:cfg.phases[0],ms:0},{label:cfg.phases[1],ms:b.departure},{label:cfg.phases[2],ms:b.settle},{label:cfg.phases[3],ms:b.approach},{label:cfg.phases[4],ms:b.arrival}]}
  ...
  function audioRoleFor(ms,total,cfg){
    if(ms>=total)return null;const b=boundaries(total,cfg),departureFade=Math.min(cfg.fx[1].roleFadeSeconds*1000,b.departure);
  ...
  async function ensureNodes(){
    ...
    if(cfg.event&&!eventNodes[activeJourneyId]){
      ...
      else{bucket.primary=makeMediaNode(cfg.event.url,{loop:false,preload:'none'});bucket.primary.gain.gain.value=.08}
  ...
  async function activateRole(role,ms,total,fadeSeconds){
    ...
    if(previous&&previous!==role)for(const key of cfg.roles[previous]){const old=nodes[key];old?.gain.gain.setTargetAtTime(0,ctx.currentTime,fadeSeconds/3);setTimeout(()=>{if(!cfg.roles[audibleRole]?.includes(key))old?.el.pause()},fadeSeconds*1000)}
  ...
  function updateAudio(ms){
    const cfg=config();if(!cfg||!ctx)return;const total=Math.max(60000,durationMinutes*60000),role=audioRoleFor(ms,total,cfg),roleChanged=role!==audibleRole,fx=transitionProfile(ms,total,cfg),fadeSeconds=role==='bed'?cfg.fx[1].roleFadeSeconds:role==='arrival'?cfg.fx[4].roleFadeSeconds:fx.roleFadeSeconds;void activateRole(role,ms,total,fadeSeconds).catch(console.error);
    const factor=(.88+.18*macro.engine)*(.96+.05*macro.activity)*(.96+.04*macro.turbulence)*(1-.08*macro.night);
    for(const [key,node] of Object.entries(activeNodes())){const on=role&&cfg.roles[role].includes(key),gain=on?cfg.gains[key]*(role==='bed'?factor:1)*fx.gain:0;node.gain.gain.setTargetAtTime(gain,ctx.currentTime,(roleChanged?fadeSeconds:fx.gainSeconds)/3);node.filter.frequency.setTargetAtTime(fx.cutoff,ctx.currentTime,fx.filterSeconds/3)}
    const eventNode=eventNodes[activeJourneyId]?.primary;if(eventNode)eventNode.gain.gain.setTargetAtTime(.08*(.65+.35*macro.activity)*(1-.08*macro.night),ctx.currentTime,.5);
  }
  ...
  async function playHoodEvent(type,distance=Math.random(),panOverride=null){
    if(activeJourneyId!=='hood_journey'||!scenePlaying||!window.LullabyJourneyEvents?.enabled)return;const node=hoodEventNode(type);if(!node)return;const gun=hoodGuns.includes(type),voice=hoodVoices.includes(type),sirenType=hoodSirens.includes(type),minimumDistance=gun||voice||type==='glass'?0.58:0,d=Math.max(minimumDistance,Math.min(1,distance)),close=1-d,base=gun?0.13:voice?0.12:sirenType?0.46:{glass:.10,footsteps:.08,carPass:.12,carDoor:.10,helicopter:.09,dog:.08}[type]||.1,range=gun?0.20:voice?0.18:sirenType?0.20:{glass:.18,footsteps:.11,carPass:.18,carDoor:.17,helicopter:.13,dog:.14}[type]||.15;
    node.filter.frequency.setValueAtTime((gun||voice?2600:3200)+close*(gun||voice?5200:10800),ctx.currentTime);node.gain.gain.setValueAtTime((base+close*range)*(.78+.35*macro.activity),ctx.currentTime);if(node.panner){const direction=Math.random()<.5?-1:1;node.panner.pan.cancelScheduledValues(ctx.currentTime);node.panner.pan.setValueAtTime(panOverride==null?(type==='carPass'?0.92*direction:random(-.85,.85)):Math.max(-1,Math.min(1,panOverride)),ctx.currentTime);if(type==='carPass')node.panner.pan.linearRampToValueAtTime(-.92*direction,ctx.currentTime+6.25)}try{node.el.currentTime=0;currentEventLabel=hoodLabels[type];eventLabelUntil=performance.now()+(config().event.sources[type]?.[1]||2500);await node.el.play();if(sirenType)document.dispatchEvent(new CustomEvent('lullaby-hood-siren',{detail:{distance:d,durationMs:15000,direction:Math.random()<.5?-1:1}}))}catch(error){console.warn(`hood ${type} unavailable`,error)}
  }
  function startHoodFight(intensity){
    const left={distance:random(.58,.94),pan:random(-.78,-.30)},right={distance:random(.58,.96),pan:random(.30,.78)};
    ...
    let elapsed=random(120,481),basicRemaining=basicShotCount,shotgunRemaining=shotgunCount,volley=0;
    while(basicRemaining>0){
      const side=volley++%2?right:left,groupSize=randomInt(1,Math.min(3,basicRemaining)+1);
    ...
    while(shotgunRemaining>0){
      elapsed+=random(900,1901);const side=volley++%2?right:left,at=elapsed;later(()=>void playHoodEvent('gunShotgun',side.distance,side.pan),at);shotgunRemaining--;
    }
    const lastGunshotMs=elapsed;
    if(Math.random()<.60){const type=hoodVoices[randomInt(0,hoodVoices.length)],at=lastGunshotMs+random(450,1801);later(()=>void playHoodEvent(type,random(.62,1),random(-.72,.72)),at)}
    if(Math.random()<.42){const at=lastGunshotMs+random(700,2801);later(()=>void playHoodEvent('glass',random(.62,1),random(-.76,.76)),at)}
    const sirenCount=randomInt(1,4),delays=Array.from({length:sirenCount},()=>lastGunshotMs+random(18000,115001)).sort((a,b)=>a-b);delays.forEach((delay,index)=>later(()=>void playHoodEvent(hoodSirens[index],random(.18,.66)),delay));
  }
  function scheduleHoodEvent(delayMs){const cfg=config(),id=activeJourneyId,event=cfg?.event;if(id!=='hood_journey'||!event)return;later(()=>{if(activeJourneyId!==id||!scenePlaying||!window.LullabyJourneyEvents?.enabled)return;const index=phaseIndex(currentElapsed(),Math.max(60000,durationMinutes*60000),cfg);if(!event.phases.includes(index)){scheduleHoodEvent(90000);return}const intensity=.35+.65*macro.turbulence;if(Math.random()<.26+.18*intensity)startHoodFight(intensity);else{const calm=['footsteps','carDoor','dog','helicopter','glass'],type=calm[Math.floor(Math.random()*calm.length)];void playHoodEvent(type,random(type==='glass'?0.58:.25,1))}scheduleHoodEvent(random(event.minMs,event.maxMs))},delayMs??random(event.minMs,event.maxMs))}
  ...
  async function debugTriggerEvent(type='random'){
    ...
    if(activeJourneyId==='hood_journey'){
      ...
      if(type!=='random'){await playHoodEvent(type,type==='siren'?random(.18,.66):random(.58,1));return local(hoodLabels[type]||[type,type])}
      const intensity=.35+.65*macro.turbulence;if(Math.random()<.55){startHoodFight(intensity);return'HOOD random fight'}
      const calm=['footsteps','carPass','carDoor','dog','helicopter','glass'],picked=calm[randomInt(0,calm.length)];await playHoodEvent(picked,random(picked==='carPass'?.18:.58,1));return local(hoodLabels[picked])
    }
