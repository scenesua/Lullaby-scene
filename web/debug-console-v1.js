(()=>{
  ...
  function installFreshBridge(){
    return new Promise((resolve,reject)=>{const doc=frame.contentDocument;if(!doc?.body){reject(new Error('디버그 플레이어 문서를 열 수 없습니다.'));return}doc.getElementById('lullabyDebugBridgeForceV4')?.remove();const script=doc.createElement('script');script.id='lullabyDebugBridgeForceV4';script.src=`/debug-bridge-v1.js?v=4&force=${Date.now()}`;script.onload=()=>{patchKoreanSutraLabel();resolve()};script.onerror=()=>reject(new Error('최신 디버그 브리지를 불러오지 못했습니다.'));doc.body.appendChild(script)})
  }
  ...
  function bind(){
    document.addEventListener('click',event=>{const button=event.target.closest('[data-command]');if(!button)return;const command=button.dataset.command;if(command==='play')run('재생/일시정지',debug=>debug.playPause());else if(command==='stop')run('정지',debug=>debug.stop());else if(command==='event')run('랜덤 이벤트 즉시 실행',debug=>debug.triggerEvent($('#eventType').value));else if(command==='loop')run('다음 루프 직전 이동',debug=>debug.jumpBeforeLoop(Number($('#loopLead').value)));else if(command==='prev')run('이전 단계',debug=>debug.previousStage());else if(command==='next')run('다음 단계',debug=>debug.nextStage());else if(command==='short')run('5분 압축 여정',debug=>debug.setDuration(5));else if(command==='resume')run('오디오 컨텍스트 재개',debug=>debug.setAudioContext(true));else if(command==='suspend')run('오디오 컨텍스트 정지',debug=>debug.setAudioContext(false));else if(command==='clear')run('로그 초기화',debug=>debug.clearLogs());else if(command==='reload'){frame.src='/player/?debug=1&reload='+Date.now();notify('플레이어를 다시 불러오는 중…')}});
