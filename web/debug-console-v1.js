(()=>{
  ...
  function render(){
    ...
    const percent=Math.round(data.progress*1000)/10;state.innerHTML=`<div><small>여정</small><strong>${data.journeyId}</strong></div><div><small>단계</small><strong>${data.phase||'—'}</strong></div><div><small>재생</small><strong>${data.playing?'RUNNING':'STOPPED'}</strong></div><div><small>시간</small><strong>${formatMs(data.elapsedMs)} / ${formatMs(data.totalMs)}</strong></div><div><small>진행률</small><strong>${percent}%</strong></div><div><small>AudioContext</small><strong>${data.audioContext}</strong></div><div><small>랜덤 이벤트</small><strong>${data.eventsEnabled?'ON':'OFF'}</strong></div><div><small>현재 이벤트</small><strong>${data.event||'—'}</strong></div>`;
