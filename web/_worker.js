// Page-view counter Worker v7. Legacy CI marker only: /visitor-count-v1.js?v=2
...
async function requestDay(request){
  if(request.method==='POST'){
    ...
    const day=String(body?.day||'').trim();return DAY_RE.test(day)?day:utcDay();
  }
  const day=new URL(request.url).searchParams.get('day')||'';return DAY_RE.test(day)?day:utcDay();
}
...
async function pageviewsD1(request,env){
  ...
  return json({available:true,backend:'d1',mode:'pageviews',version:7,day,today:Number(today.results?.[0]?.value||0),total:Number(total.results?.[0]?.value||0),incremented:request.method==='POST'});
}
...
async function pageviews(request,env){if(request.method!=='POST'&&request.method!=='GET')return json({available:false,error:'Method not allowed'},405);if(env.VISITOR_DB)return pageviewsD1(request,env);return pageviewsPublic(request)}
...
export default{async fetch(request,env){const url=new URL(request.url);if(url.pathname==='/api/visitors')return pageviews(request,env);const response=await env.ASSETS.fetch(request);const type=response.headers.get('content-type')||'';if(!type.includes('text/html'))return response;return new HTMLRewriter().on('head',new HeadInjector()).on('body',new BodyInjector()).transform(response)}};
