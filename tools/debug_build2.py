...
# Create a Forest Temple-only widened bamboo ambience. The global bamboo source stays untouched.
src = Path("web/audio/bamboo_forest.ogg")
out = Path("web/audio/scenes/forest_temple_journey/forest_temple_bamboo_wide_bed_001.ogg")
subprocess.run([
    "ffmpeg", "-hide_banner", "-loglevel", "error", "-y", "-i", str(src),
    "-af", "extrastereo=m=1.35:c=1",
    "-ar", "48000", "-ac", "2", "-c:a", "libopus", "-b:a", "112k", "-vbr", "on", str(out),
], check=True)
if not out.exists() or out.stat().st_size == 0:
    raise SystemExit("wide bamboo render failed")

p = Path("web/remaining-journeys-v1.js")
s = p.read_text()
s = once(s,
    "forest:['/audio/bamboo_forest.ogg?v=1',94.744]",
    "forest:['/audio/scenes/forest_temple_journey/forest_temple_bamboo_wide_bed_001.ogg?v=1',94.744]",
    "wide bamboo source")
s = once(s,
    "gains:{departure:.34,forest:.215,bowl:.227}",
    "gains:{departure:1,forest:.152,bowl:.227}",
    "stage one footsteps and bamboo level")
s = once(s,
...
marker = "  function scheduleTempleEvent(delayMs){"
if marker not in s:
    raise SystemExit("missing temple scheduler marker")
helper = """  async function playTempleEventNow(type,{resumeRandom=false}={}){
...
old = "    const cfg=config();if(!cfg||!ctx)return;const total=Math.max(60000,durationMinutes*60000),role=audioRoleFor(ms,total,cfg),roleChanged=role!==audibleRole,fx=transitionProfile(ms,total,cfg),fadeSeconds=role==='bed'?cfg.fx[1].roleFadeSeconds:role==='arrival'?cfg.fx[4].roleFadeSeconds:fx.roleFadeSeconds;void activateRole(role,ms,total,fadeSeconds).catch(console.error);"
...
s = once(s, old, new, "stage 3 forced sutra hook")
s = once(s,
    "function stop(arrived=false){scenePlaying=false;pausedAt=0;",
    "function stop(arrived=false){scenePlaying=false;pausedAt=0;templePhaseIndex=-1;templeStageThreeStarting=false;",
    "reset temple phase state")
p.write_text(s)

p = Path("web/player/index.html")
s = p.read_text()
s = once(s, "/remaining-journeys-v1.js?v=31", "/remaining-journeys-v1.js?v=32", "player cache key")
p.write_text(s)

p = Path("web/sw.js")
s = p.read_text()
s = once(s, "const CACHE='lullaby-scene-debug-v18'", "const CACHE='lullaby-scene-debug-v19'", "service worker cache")
s = once(s, "/remaining-journeys-v1.js?v=31", "/remaining-journeys-v1.js?v=32", "service worker journey key")
p.write_text(s)

p = Path("web/debug/index.html")
s = p.read_text()
s = once(s, "DEBUG_BUILD:1", "DEBUG_BUILD:2", "debug build marker")
s = once(s,
    '현재 디버그 버전 · <strong style="font-size:18px;color:#fff">#1</strong>',
    '현재 디버그 버전 · <strong style="font-size:18px;color:#fff">#2</strong>',
    "debug build banner")
p.write_text(s)

print("Forest Temple debug build 2 prepared")
