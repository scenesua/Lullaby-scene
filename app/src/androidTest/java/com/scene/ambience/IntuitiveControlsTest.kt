        ...
        try {
            for (largeText in listOf(false, true)) {
                ...
                ActivityScenario.launch(MainActivity::class.java).use {
                    ...
                    if (device.findObject(switchSelector).isChecked) device.findObject(switchSelector).click()
                    assertNotNull(device.wait(Until.findObject(switchSelector.checked(false)), 10_000))
                    device.findObject(By.desc(name).checkable(true)).click()
                    ...
                    val slider = device.findObject(By.desc(name).clazz("android.widget.SeekBar"))
                    assertNotNull("Native slider remains accessible", slider)
