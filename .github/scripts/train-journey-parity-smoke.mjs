...
const remainingRuntime=text('web/remaining-journeys-v1.js');
const debugForestHash='c92ff41bfecf55166d8cdfea5a160572d83a4f44d433964251ed7032dac360f9';
...
const expected={
  ...
  ferry_journey_departure_001:['ferry_journey','continuous',128667,'b01b3d63105b9439a728fe06222dab030a0ce5e516b84fa31aa77cd8543a4dc4'],ferry_journey_bed_001:['ferry_journey','continuous',120666,'5583c17eaffda0255f70a12be33634bd0a3e11c05274b203d9e5eb62f958a211'],ferry_journey_arrival_001:['ferry_journey','continuous',98065,'6439ec0e9029126670bcf94b576261e0dbf40e304300b27221e6268b58a51932'],
  ...
  forest_temple_forest_bed_001:['forest_temple_journey','continuous',73220,'7c242464781c044600c28bb98192155e06e50a242ffb5a1119bb17e1685543d3'],forest_temple_korean_distant_birds_bed_001:['forest_temple_journey','continuous',120030,'1e74a03d24f020ca7a208a5335f1902abc3488d6fb3e46660ab2e7f12224aeeb'],forest_temple_heart_sutra_event_001:['forest_temple_journey','events',120196,'30c52dd70ba9d28f5fdf52e83156c3cd281450bc256c2bbd944c04a6d4034533']
};
...
for(const [assetId,[scene,kind,duration,hash]] of Object.entries(expected)){
  ...
  if(assetId==='forest_temple_forest_bed_001'&&remainingRuntime.includes('forest_temple_forest_bed_001.ogg?v=3')){
    if(androidHash!==hash||webHash!==debugForestHash)throw new Error(`${assetId} explicit debug-preview hash mismatch`);
    continue;
  }
...
if(!web.includes('fadeSeconds:9.48'))throw new Error('Web train bed crossfade is not aligned to the wheel rhythm');
const trainBed=manifest.sources.find(source=>source.id==='train_journey_bed')?.continuous?.[0];
if(trainBed?.crossfade_ms!==9480)throw new Error('Android train bed crossfade is not aligned to the wheel rhythm');
...
if(!text('web/player/index.html').includes('/remaining-journeys-v1.js?v=32'))throw new Error('Web player does not load remaining Journey runtime');
...
if(!text('web/player/index.html').includes('/journey-background-v1.js?v=17'))throw new Error('Web player does not load Journey backgrounds');
...
for(const marker of ['SOURCE_FOREST_TEMPLE_BOWL','SOURCE_FOREST_TEMPLE_KOREAN_BIRDS','SOURCE_FOREST_TEMPLE_MOKTAK','SOURCE_FOREST_TEMPLE_GRAVEL','SOURCE_FOREST_TEMPLE_HEART_SUTRA','startTempleEvents'])if(!orchestrator.includes(marker))throw new Error(`Android Forest Temple runtime missing ${marker}`);
for(const marker of ["forest_temple_path_walk_001.ogg?v=1","forest_temple_forest_bed_001.ogg?v=3',226","forest_temple_korean_distant_birds_bed_001.ogg?v=1","roles:{departure:['departure','forest','birds'],bed:['forest','birds','bowl']","bedStartsAtPhaseBoundary:true","forestCrossfade=activeJourneyId==='forest_temple_journey'&&key==='forest'","fadeSeconds:forestCrossfade?12:8","const templeEventTypes=['moktak','moktak','heartSutra']","scheduleTempleFootsteps","stage===1?random(38000,76000)","delete configs.hood_journey;configs.hood_journey=hoodConfig","'forest_temple_journey','hood_journey'"])if(!remaining.includes(marker))throw new Error(`Web Forest Temple order/runtime missing ${marker}`);
